...
return val;},setVal:function(data){var val={};for(name in this._properties){var key=this._schema?this._schema.mapName(name):name;val[key]=this._properties[name].value=data?data[key]:null;}
...
for(i=0;record.addedNodes&&i<record.addedNodes.length;i++){bindDOMTree(record.addedNodes[i],self.root,self.dataRoot,{noUnbind:true});}});}});DD.ListFactory=ListFactory;DD.TemplateListFactory=TemplateListFactory;DD.BindingScope=BindingScope;document.addEventListener("DOMContentLoaded",function(){if(DD.settings.autobind!=false&&DOMObserverClass){new BindingScope().bind();}});}(DataDrive);!function(DD){"use strict";var AjaxConnect=function(model,url,ajaxOpts){var settings=jQuery.extend({accepts:'application/json'},ajaxOpts||{});var successHandler=settings.success;var modelUpdate=settings.modelUpdate;var dataConvert=settings.dataConvert;delete settings.modelUpdate;delete settings.dataConvert;settings.success=function(data){if(typeof(modelUpdate)=='function'){modelUpdate.call(model,data);}else{if(typeof(dataConvert)=='function'){data=dataConvert(data);}
model.value=data;}
