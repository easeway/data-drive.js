/* Copyright (c) 2013, Yisui Hu <easeway@gmail.com> */!function(exports){var DD={VERSION:"2.0.0"};DD.defClass=function(base,proto){if(typeof(base)!='function'){proto=base;base=Object;}
if(!proto){proto={};}
...
return value;},undefined);};}}});DD.extensions=new Extensions();}(DataDrive);!function(DD){"use strict";var Value=DD.defClass(DD.Observer,{constructor:function(){DD.Observer.prototype.constructor.call(this);this.caching=false;this.cachedRev=null;this.cachedVal=null;this.valueRev=0;},get value(){if(!this.caching){return this.getVal();}
if(this.cachedRev===this.valueRev){return this.cachedVal;}
this.cachedVal=this.getVal();this.cachedRev=this.valueRev;return this.cachedVal;},set value(val){var oldVal=this.getVal();var newVal=this.setVal(val);this.flush({change:'update',oldVal:oldVal,newVal:newVal});},query:function(path,callback){var names;if(Array.isArray(path)){names=path;}else{names=path.split(".");}
...
return left?null:value;},flush:function(notification){if(this.caching){this.cachedVal=null;this.valueRev++;}
if(notification){notification.data=this;this.notify(notification);}},refresh:function(){this.notify({change:'update',refresh:true,data:this});}});var Scalar=DD.defClass(Value,{constructor:function(){Value.prototype.constructor.call(this);this._value=null;},getVal:function(){return this._value;},setVal:function(val){return this._value=val;}});var List=DD.defClass(Value,{constructor:function(itemType){Value.prototype.constructor.call(this);this.isList=true;this.itemType=itemType;this.items=[];},findProperty:function(name){var index=parseInt(name);return isNaN(index)?null:this.items[index];},getVal:function(){return this.items.map(function(item){return item.value;});},setVal:function(items){if(Array.isArray(items)){var val=[];this.items=this._createItems(items,function(v){val.push(v);});return val;}else if(items===null||items===undefined){this.items=[];return[];}
throw new TypeError("items is not an Array");},get length(){return this.items.length;},at:function(index){return this.items[index];},valAt:function(index){var item=this.at(index);return item?item.value:undefined;},update:function(index,items){if(items&&!Array.isArray(items)){items=[items];}
...
if(params.dataSource&&params.dataSource[0]!='.'){this.data=dataRoot.query(params.dataSource);}else{var data=null;for(var elem=this.element;elem&&elem!=root.parentElement;elem=elem.parentElement){if(elem[BINDING_PROP]){data=elem[BINDING_PROP].data;break;}}
...
Binding.create=function(node,root,dataRoot){var params={};if(node.nodeType==1){var attr=node.attributes.getNamedItem(ATTR_MAP);if(attr){params.dataSource=attr.value.trim();params.node=node;}
...
if(count==0){params.changes=null;}}
params.script=buildFunction(attr.value);params.node=node;}
...
params.node=node;params.element=node.parentElement;}}
if(params.element){return new Binding(root,dataRoot,params);}
...
throw new DD.NotImplementedError("createItemNodes not implemented");},_createItemViews:function(items){return items.map(function(item){var itemView={data:item,nodes:this.createItemNodes()};itemView.nodes.forEach(function(node){bindDOMTree(node,node,item,{});});item.refresh();return itemView;},this);},_findDirectChild:function(node){while(node&&node.parentElement!=this.container){node=node.parentElement;}
return node;},_insertItemViewFn:function(index){var at=new SparseIndex(this.items).closest(index);var refNode=this._findDirectChild(at!=undefined?this.items[at].nodes[0]:null);var insertNodeFn=refNode?function(node){this.container.insertBefore(node,refNode);}:function(node){this.container.appendChild(node);};return function(view){view.nodes.forEach(function(node){insertNodeFn.call(this,node);},this);}},_removeItemView:function(view){view.nodes.forEach(function(node){var child=this._findDirectChild(node);if(child){this.container.removeChild(child);}},this);}});var InlineListFactory=DD.defClass(ListFactory,{constructor:function(binding){ListFactory.prototype.constructor.call(this,binding);var children=this.container.childNodes;this.nodesTemplate=[];for(var i=0;i<children.length;i++){this.nodesTemplate.push(children[i]);}
this.container.innerHTML="";}});function autoSelectListFactory(binding){return new InlineListFactory(binding);}
function unbindNode(node){var binding=node[BINDING_PROP];if(binding){binding.unbind();delete node[BINDING_PROP];}}
function bindNode(node,root,dataRoot,opts){if(!opts.noUnbind){unbindNode(node);}else if(node[BINDING_PROP]){return true;}
var binding=Binding.create(node,root,dataRoot);if(binding){Object.defineProperty(node,BINDING_PROP,{get:function(){return binding;},configurable:true,enumerable:false});return true;}
return false;}
function traverseDOMTree(node,callback){switch(node.nodeType){case 1:callback(node);for(var i=0;i<node.childNodes.length;i++){traverseDOMTree(node.childNodes[i],callback);}
break;case 3:callback(node);break;}}
function unbindDOMTree(node){traverseDOMTree(node,function(node){unbindNode(node);});}
function bindDOMTree(node,root,dataRoot,opts){traverseDOMTree(node,function(node){bindNode(node,root,dataRoot,opts);});}
var DOMObserverClass=window.MutationObserver;if(!DOMObserverClass){DOMObserverClass=window.WebKitMutationObserver;}
if(!DOMObserverClass){console.error("Your browser doesn't support DOM MutationObserver");}
DD.compatibility.register("MutationObserver",function(results){return DOMObserverClass;});var BindingScope=DD.defClass({constructor:function(node,dataRoot){var self=this;this.observer=new DOMObserverClass(function(){self.onMutations.apply(self,arguments);});this.scope(node,dataRoot);},refresh:function(opts){var _opts={noUnbind:true};for(var k in opts){_opts[k]=opts[k];}
bindDOMTree(this.root,this.root,this.dataRoot,_opts);return this;},scope:function(node,dataRoot){this.unbind();this.root=node||document.body;this.dataRoot=dataRoot||DD.Models;return this;},bind:function(refresh){this.observer.observe(this.root,{childList:true,attributes:true,characterData:true,subtree:true});return refresh===false?this:this.refresh();},unbind:function(){this.observer.disconnect();if(this.root){unbindDOMTree(this.root);}
return this;},onMutations:function(mutations){var self=this;mutations.forEach(function(record){var i;for(i=0;record.removedNodes&&i<record.removedNodes.length;i++){unbindDOMTree(record.removedNodes[i]);}
for(i=0;record.addedNodes&&i<record.addedNodes.length;i++){bindDOMTree(record.addedNodes[i],self.root,self.dataRoot,{noUnbind:true});}});}});DD.ListFactory=ListFactory;DD.InlineListFactory=InlineListFactory;DD.BindingScope=BindingScope;if(DD.settings.autobind!=false&&DOMObserverClass){document.addEventListener("DOMContentLoaded",function(){new BindingScope().bind();});}}(DataDrive);!function(DD){var AjaxConnect=function(model,url,ajaxOpts){var settings=jQuery.extend({accepts:'application/json'},ajaxOpts||{});var successHandler=settings.success;settings.success=function(data){model.setVal(data);if(typeof(successHandler)=='function'){successHandler.apply(this,arguments);}};model.reload=function(){return jQuery.ajax(url,settings);};return model;};DD.AjaxConnect=AjaxConnect;DD.extensions.connect("Schema",{modeling:function(model,schema){if(schema.options.ajax&&schema.options.ajax.url){AjaxConnect(model,schema.options.ajax.url,schema.options.ajax.settings);}}});}(DataDrive);