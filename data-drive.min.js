/* Copyright (c) 2013, Yisui Hu <easeway@gmail.com> */!function(exports){"use strict";var DD={VERSION:"2.1.0"};DD.defClass=function(base,proto){if(typeof(base)!='function'){proto=base;base=Object;}
if(!proto){proto={};}
...
return this;},refresh:function(){return this.notify({change:'update',refresh:true,data:this});}});function valueOrFunction(val){if(typeof(val)=='function'){return val();}
return val;}
var Scalar=DD.defClass(Value,{constructor:function(defaultVal,initialVal){Value.prototype.constructor.call(this);this.__s.defaultVal=defaultVal==undefined?null:defaultVal;this.__s.value=valueOrFunction(initialVal==undefined?this.__s.defaultVal:initialVal);},getVal:function(){return this.__s.value;},setVal:function(val){return this.__s.value=(val==undefined||val==null)?valueOrFunction(this.__s.defaultVal):val;}});var List=DD.defClass(Value,{constructor:function(itemType){Value.prototype.constructor.call(this);this.isList=true;this.itemType=itemType;this.items=[];},findProperty:function(name){var index=parseInt(name);return isNaN(index)?null:this.items[index];},getVal:function(){return this.items.map(function(item){return item.value;});},setVal:function(items){if(Array.isArray(items)){var val=[];this.items=this._createItems(items,function(v){val.push(v);});return val;}else if(items===null||items===undefined){this.items=[];return[];}
...
var Binding=DD.defClass(DD.Listener,{constructor:function(root,scope,params){DD.Listener.prototype.constructor.call(this);this.root=root;this.scope=scope;this.element=params.element;this.node=params.node;this.options=params.options||{};this.handlers=[];if(params.contents){this.contents=params.contents;this.handlers.push(function(change){this.node.textContent=this.contents.evaluate(this,change);});}else{if(params.attrs){this.attrs=params.attrs;this.handlers.push(function(change){for(var key in this.attrs){this.node.setAttribute(key,this.attrs[key].evaluate(this,change));}});}
if(typeof(params.script)=='function'){this.script=params.script;this.changes=params.changes;this.handlers.push(function(change){if(!this.changes||this.changes[change.change]){this.script.call(this,change,this.data,this.element,this.node);}});}}
...
this.handlers.forEach(function(handler){handler.call(this,change);},this);},});function buildFunction(script){var fn;try{return eval("fn = function ($C, $D, $E, $N) { "+script+" }");}catch(e){console.error("Bad script: "+script);console.error(e);throw e;}}
function buildStatement(script){var fn;try{return eval("fn = function ($C, $D, $E, $N) { return ( "+script+" ); }");}catch(e){console.error("Bad script: "+script);console.error(e);throw e;}}
var SUBST_REGEX=/%\{[^\}]+\}/g;var ContentTemplate=DD.defClass({constructor:function(text){if(text){this.parse(text);}else{this.contents=[];}},parse:function(text){this.contents=[];SUBST_REGEX.lastIndex=0;var start=0,m,count=0;while((m=SUBST_REGEX.exec(text))!=null){var scriptBegin=m.index;if(scriptBegin>start){this.contents.push(text.substr(start,scriptBegin-start));}
start=SUBST_REGEX.lastIndex;this.contents.push(buildStatement(m[0].substr(2,m[0].length-3)));count++;}
if(count>0){if(start<text.length){this.contents.push(text.substr(start));}}
return count>0;},get valid(){return this.contents.length>0;},evaluate:function(binding,change){return this.contents.reduce(function(text,content){if(typeof(content)=='function'){text+=content.call(binding,change,binding.data,binding.element,binding.node);}else{text+=content;}
return text;},"");}});Binding.create=function(node,root,scope,opts){var params={};if(opts.data){params.data=opts.data;}
if(node.nodeType==1){for(var n=0;n<node.attributes.length;n++){var attr=node.attributes[n];switch(attr.name){case ATTR_MAP:params.dataSource=attr.value.trim();params.node=node;break;case ATTR_ON:{var i=attr.value.indexOf(':');if(i>=0){params.script=buildFunction(attr.value.substr(i+1));params.changes={};var changes=attr.value.substr(0,i).split(',');var count=0;for(var i=0;i<changes.length;i++){var name=changes[i].trim();if(name==="*"){params.changes=null;break;}else if(name!=""){params.changes[name]=true;count++;}}
if(count==0){params.changes=null;}}else{params.script=buildFunction(attr.value);params.changes=null;}
params.node=node;break;}
case ATTR_OPTS:params.options=attr.value.split(';').reduce(function(result,item){var i=item.indexOf(':');if(i>0){result[item.substr(0,i).trim()]=item.substr(i+1).trim();}
return result;},{});if(params.options.bind){params.node=node;}
break;case ATTR_LIST:params.listFactory=buildStatement(attr.value);break;default:if(attr.name.substr(0,ATTR_ATTR.length)==ATTR_ATTR){var attrName,contents;if((attrName=attr.name.substr(ATTR_ATTR.length)).length>0&&(contents=new ContentTemplate(attr.value)).valid){if(!params.attrs){params.attrs={};}
params.attrs[attrName]=contents;params.node=node;}}}}
if(params.data){params.node=node;}
if(params.node){params.element=node;}}else{var contents=new ContentTemplate(node.textContent);if(contents.valid){params.contents=contents;}
if(params.contents||params.data){params.node=node;params.element=node.parentElement||opts.container;}}
