...
if(params.data){this.data=params.data;}else if(params.dataSource&&params.dataSource[0]!='.'){this.data=dataRoot.query(params.dataSource);}else{var data=null;for(var elem=this.element;elem&&elem!=root.parentElement;elem=elem.parentElement){if(elem[BINDING_PROP]){data=elem[BINDING_PROP].data;break;}}
...
Binding.create=function(node,root,dataRoot,opts){var params={};if(opts.data){params.data=opts.data;}
...
if(count>0||params.data){params.node=node;params.element=node.parentElement;}}
...
throw new DD.NotImplementedError("createItemNodes not implemented");},_createItemViews:function(items){return items.map(function(item){var itemView={data:item,nodes:this.createItemNodes()};itemView.nodes.forEach(function(node){bindDOMTree(node,node,this.binding.dataRoot,{data:item});},this);item.refresh();return itemView;},this);},_findDirectChild:function(node){while(node&&node.parentElement!=this.container){node=node.parentElement;}
...
var binding=Binding.create(node,root,dataRoot,opts);if(binding){Object.defineProperty(node,BINDING_PROP,{get:function(){return binding;},configurable:true,enumerable:false});return true;}
return false;}
...
function bindDOMTree(node,root,dataRoot,opts){traverseDOMTree(node,function(node,level){if(level>0){delete opts.data;}
...
return this;},findBinding:function(node){for(;node&&node!=this.root;node=node.parentElement){if(node[BINDING_PROP]){return node[BINDING_PROP];}}
